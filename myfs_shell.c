...
int main(){
	...
	while(1)
	{
		...
		else // "my"가 없으면
		{
			...
			else if (strcmp(command_option[0], "myfindinode") == 0)
				printf("%d번째 아이노드에 할당되어있다.\n",find_file_inode(&m, command_option[1]));
...
void call_myls(struct myfs* m,char command_option[6][15]) {
	...
	if(command_option[2][0]!=0) strcpy(name_or_path,command_option[1]);
	//name_or_path가 0인지 아닌지 따짐 (현재디렉인가 아닌가)
	//if(!=0)이면 아이노드 구하기 위한 여정 시작
	//name_or_path path인지 아닌지 따지는 함수
...
void call_mytouch(char command_option[6][15], struct myfs* m) {
	...
	if (find_file_inode(m, file_name) != 0){		//파일이 존재하면...
		m->inodelist[find_file_inode(m, file_name)].n = now_time();
	}
	else{
		void_inode = allocation_file_inode(m, file_name, 0);		//dir_name 이름을 가진 파일을 아이노드와 데이터블록에 할당해주는 함수
		void_block = m->inodelist[void_inode].direct = print_super_block(m);
	...
	if(!check)		//겹치는 게 없다면
	{
		...
		if(at_direct == 0){   //새로 다이렉트 블록을 할당해야 하는 경우
			...
			for(int i=at_single*10;i<at_single*10+10;i++){
				if(direct_num>>(i%10)&1==1)
					m->datablock[single_num].si.block[i/32].n += pow(2,(i%32));
			}
		}

		else{	//가장 끝에 있는 다이렉트 블록을 읽어와야 할 경우
			for(int i=at_single*10;i<at_single*10+10;i++){
				if(m->datablock[single_num].si.block[i/32].n>>(i%32)&1==1)
...
void show_files(struct myfs *m){
	for(int i=0;i<19;i++){
		printf("name:");
		for(int j=0;j<4;j++)
			printf("%c",m->datablock[0].d.files[i].name[j]);
		printf("inode:%d",m->datablock[0].d.files[i].inode);
		printf("\n");
	}
}
...
void rm_file_inode (struct myfs * m,char name[4],int flag_d_f) { // file이름 받아 indoe할당받고 저장, file이름도
	...
	if(check)
	{
		if(m->inodelist[check].single_indirect){
			...
			for(int k=0;k<21;k++){
				if(m->datablock[last_block].df.files[k].name[0]==0){
					last_file=k-1;
					break;
				}
			}  //그 폴더의 마지막 파일을 찾음
		...
		if(direct_empty&&m->inodelist[check].single_indirect==0){ //다이렉트에 빈 파일이 생길 경우&&single없음
			m->datablock[now_dir_datablock].d.files[direct_empty].inode = m->datablock[last_block].d.files[last_file].inode;
			m->datablock[last_block].d.files[last_file].inode=0;
			strncpy(m->datablock[now_dir_datablock].d.files[direct_empty].name,m->datablock[last_block].d.files[last_file].name,4); // 이름 복사
			for(int i=0;i<4;i++)
				m->datablock[last_block].d.files[last_file].name[i]=0;
			return;
		}
		else if(direct_empty&&m->inodelist[check].single_indirect!=0){ //다이렉트에 빈 파일이 생길 경우&&single없음
			...
			for(int i=0;i<4;i++)
				m->datablock[last_block].df.files[last_file].name[i]=0;
			return;
		}
			...
			for(i = b.front;i!=NULL;i = i->next){
				for(j=0;j<21;j++){
					if(m->datablock[i->num].df.files[j].inode == check){
						for(int k=0 ; k<4 ; k++)
							m->datablock[i->num].d.files[j].name[k]='\0'; // 이름 삭제
						m->datablock[i->num].d.files[j].inode = 0;  // 아이노드 입력
						break;
					}
				}
